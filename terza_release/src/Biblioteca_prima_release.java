	...
	System.out.println(date);
        ...
        while(menu.getStatus() == 1)
        {
            ...
            else if(is_user && is_logged)
            {
                ...
                if(intScelta > 0 && intScelta <= books.size() && info.isBookAvaiableForLoan(books.get(intScelta-1)))
                {
                    ...
                    if(info.addBookLoan(utente, elemento ))
                    ...
                    for(int i = 0;i < loans.size(); i++)
                    {
                        ...
                        if(info.removeLoan(utente,
                            ...
                            Menu.print("Il libro identificato con "
                                + loans.get(i).libro.getIdentifier()  
                ...
                else if(risposta.equals("6"))
                {
                    ...
                    //stampo a video tutti i libri
                    ArrayList<Loan> loans = info.getBooksLoan(utente);
                    Menu.print("identificativo risorsa - data inizio prestito - data fine prestito - disponibilita'");
                    for (int i = 0; i < loans.size(); i++) {
                        String disponibile = "";
                        if(info.isBookAvaiableForLoan(loans.get(i).libro))
