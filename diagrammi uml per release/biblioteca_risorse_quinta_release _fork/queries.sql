...
select * from loan, person, book, category where book.identifier = loan.id_book and person.username = loan.id_person
and person.username = 'default' and loan.is_active = 1

--conta quante persone hanno un libro in prestito di una categoria data, e con dato nome

select count(*) from loan, person, book, category where book.identifier = loan.id_book and book.category_ident = category.identifier and person.username = loan.id_person
and person.username = 'dada' and loan.is_active = 1 and category_ident ='sottocategoria23'
------> inserts

INSERT INTO `renew_loan`(`id_loan`,`id_book`,`start_date`,`end_date`,`quit_loan_date`,`is_active`) VALUES ('','elementoase','2018/11/11','2019/11/11',"",'1');
INSERT INTO `renew_loan`(`id_loan`,`id_book`,`start_date`,`end_date`,`quit_loan_date`,`is_active`) VALUES (12,'elementoase','2014/11/11','2015/11/11',"",'1');
